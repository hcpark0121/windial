...
mkdir -p "$PROFILE"
nohup "$BIN" --user-data-dir="$PROFILE" --remote-debugging-port=9333 --no-first-run --no-default-browser-check \
  --window-size=1440,1000 --window-position=200,60 "$URL" >/dev/null 2>&1 &
disown
sleep 3 && curl -s http://localhost:9333/json/version | head -3
